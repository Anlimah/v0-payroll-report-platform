...
function displayAllowances(allowances, payrollManager) {
  ...
  container.innerHTML = allowances
    ...
    <div style="display: flex; align-items: center; gap: 12px; padding: 8px; border: 1px solid #e2e8f0; border-radius: 6px; margin-bottom: 8px;">
      <input type="checkbox" id="allow_${allowance.id}" data-id="${allowance.id}" class="allowance-checkbox">
      <label for="allow_${allowance.id}" style="flex: 1; margin: 0;">${allowance.allowance_name}</label>
      <div style="display: flex; gap: 8px; align-items: center;">
        <input type="number" id="allow_amount_${allowance.id}" placeholder="${allowance.is_percentage ? "%" : "Amount"}" 
...
function displayDeductions(deductions, payrollManager) {
  ...
  container.innerHTML = deductions
    ...
    <div style="display: flex; align-items: center; gap: 12px; padding: 8px; border: 1px solid #e2e8f0; border-radius: 6px; margin-bottom: 8px;">
      ...
      <div style="display: flex; gap: 8px; align-items: center;">
        <input type="number" id="deduct_amount_${deduction.id}" placeholder="${deduction.is_percentage ? "%" : "Amount"}" 
...
function calculateAndDisplaySummary(payrollManager) {
  const basicSalary = Number.parseFloat(document.getElementById("basicSalaryInput").value) || 0
    ...
    <div class="card" style="background-color: #f8fafc; padding: 20px; margin-bottom: 24px;">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
        ...
        <div>
          ...
          <div style="font-size: 24px; font-weight: 700;">$${Number.parseFloat(data.totals.basic_salary).toLocaleString()}</div>
        </div>
        <div>
          <div style="font-size: 12px; color: #64748b;">Total Allowances</div>
          <div style="font-size: 24px; font-weight: 700; color: #10b981;">$${Number.parseFloat(data.totals.total_allowances).toLocaleString()}</div>
        </div>
        <div>
          <div style="font-size: 12px; color: #64748b;">Total Deductions</div>
          <div style="font-size: 24px; font-weight: 700; color: #ef4444;">$${Number.parseFloat(data.totals.total_deductions).toLocaleString()}</div>
        </div>
        <div>
          <div style="font-size: 12px; color: #64748b;">Total Net (GHS)</div>
          <div style="font-size: 24px; font-weight: 700; color: #2563eb;">GHS ${Number.parseFloat(data.totals.net_salary_ghs).toLocaleString()}</div>
        </div>
      </div>
    </div>
    ...
    <div class="card" style="background-color: #f8fafc; padding: 20px; margin-bottom: 24px;">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
        ...
        <div>
          ...
          <div style="font-size: 24px; font-weight: 700;">GHS ${(totalPayroll / (data.months.length || 1)).toLocaleString()}</div>
        </div>
      </div>
    </div>
            ...
            <tbody>
              ${
                ...
                <tr>
                  ...
                  <td style="color: #10b981;"><strong>$${Number.parseFloat(allow.total_amount).toLocaleString()}</strong></td>
                </tr>
              `,
                      )
                      .join("")
            ...
            <tbody>
              ${
                ...
                <tr>
                  ...
                  <td style="color: #ef4444;"><strong>$${Number.parseFloat(deduct.total_amount).toLocaleString()}</strong></td>
                </tr>
              `,
                      )
                      .join("")

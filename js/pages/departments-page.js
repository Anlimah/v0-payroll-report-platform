// Import or declare the CRUDManager variable here
...
class DepartmentsPage {
  constructor() {
    this.crudManager = new window.CRUDManager(window.API_ENDPOINTS.DEPARTMENTS, "department")
        ...
        <div class="card-header">
          ...
          <div style="display: flex; gap: 12px;">
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
              <input type="checkbox" id="showArchivedDepartments" ${this.showArchived ? "checked" : ""}>
              Show Archived
            </label>
      ...
      if (response.success && response.data.length > 0) {
        tbody.innerHTML = response.data
          ...
          <tr>
            ...
            <td><span class="badge ${dept.is_archived ? "badge-danger" : "badge-success"}">${dept.is_archived ? "Archived" : "Active"}</span></td>
            <td>
              <div class="action-buttons">
                <button class="btn btn-sm btn-primary" onclick="departmentsPage.editDepartment(${dept.id})">Edit</button>
                ${
                  !dept.is_archived
                ...
                <button class="btn btn-sm btn-danger" onclick="departmentsPage.deleteDepartment(${dept.id})">Delete</button>
              </div>
            </td>
          </tr>
        `,
  ...
  async saveDepartment() {
    ...
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage(response.message || "Department saved successfully", "success")
        this.closeModal()
        await this.loadDepartments()
      } else {
        this.crudManager.showMessage(response.message || "Failed to save department", "error")
      }
    } catch (error) {
      console.error("Error saving department:", error)
      this.crudManager.showMessage("An error occurred while saving the department", "error")
    }
  }
  ...
  async editDepartment(id) {
    try {
      ...
      this.crudManager.showMessage("Failed to load department details", "error")
    }
  }
  ...
  async archiveDepartment(id) {
    this.crudManager.confirmArchive(async () => {
      try {
        ...
        if (response.success) {
          this.crudManager.showMessage("Department archived successfully", "success")
          await this.loadDepartments()
        } else {
          this.crudManager.showMessage(response.message || "Failed to archive department", "error")
        }
      } catch (error) {
        console.error("Error archiving department:", error)
        this.crudManager.showMessage("An error occurred while archiving the department", "error")
      }
    })
  }
  ...
  async unarchiveDepartment(id) {
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage("Department unarchived successfully", "success")
        await this.loadDepartments()
      } else {
        this.crudManager.showMessage(response.message || "Failed to unarchive department", "error")
      }
    } catch (error) {
      console.error("Error unarchiving department:", error)
      this.crudManager.showMessage("An error occurred while unarchiving the department", "error")
    }
  }
  ...
  async deleteDepartment(id) {
    this.crudManager.confirmDelete(async () => {
      try {
        ...
        if (response.success) {
          this.crudManager.showMessage("Department deleted successfully", "success")
          await this.loadDepartments()
        } else {
          this.crudManager.showMessage(response.message || "Failed to delete department", "error")
        }
      } catch (error) {
        console.error("Error deleting department:", error)
        this.crudManager.showMessage("An error occurred while deleting the department", "error")
      }
    })
  }
}
...
window.document.getElementById("mainContent").innerHTML = window.departmentsPage.render()

class UsersPage {
  constructor() {
    this.crudManager = new window.CRUDManager(window.API_ENDPOINTS.USERS, "user")
        ...
        <div class="card-header">
          ...
          <div style="display: flex; gap: 12px;">
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
              <input type="checkbox" id="showArchivedUsers" ${this.showArchived ? "checked" : ""}>
              Show Archived
            </label>
  ...
  attachEventListeners() {
    ...
    document.getElementById("showArchivedUsers").addEventListener("change", (e) => {
      ...
      if (response.success && response.data.length > 0) {
        tbody.innerHTML = response.data
          ...
          <tr>
            ...
            <td><span class="badge ${user.is_archived ? "badge-danger" : "badge-success"}">${user.is_archived ? "Archived" : "Active"}</span></td>
            <td>
              <div class="action-buttons">
                <button class="btn btn-sm btn-primary" onclick="usersPage.editUser(${user.id})">Edit</button>
                ${
                  !user.is_archived
                ...
                <button class="btn btn-sm btn-danger" onclick="usersPage.deleteUser(${user.id})">Delete</button>
              </div>
            </td>
          </tr>
        `,

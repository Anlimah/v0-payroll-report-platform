class AllowancesPage {
  constructor() {
    this.crudManager = new window.CRUDManager(window.API_ENDPOINTS.ALLOWANCES, "allowance")
  ...
  renderTable(allowances) {
    ...
    tbody.innerHTML = allowances
      ...
      <tr>
        ...
        <td><span class="badge badge-primary">${allowance.is_percentage ? "Percentage" : "Fixed"}</span></td>
        <td>${allowance.is_percentage ? allowance.default_amount + "%" : "$" + Number.parseFloat(allowance.default_amount).toFixed(2)}</td>
        <td>${allowance.description || "-"}</td>
        <td>
          <span class="badge ${allowance.is_archived ? "badge-danger" : "badge-success"}">
            ${allowance.is_archived ? "Archived" : "Active"}
          </span>
        </td>
        <td>
          <div class="action-buttons">
            <button class="btn btn-sm btn-primary" onclick="window.allowancesPage.showEditModal(${allowance.id})" title="Edit">
              Edit
            </button>
            <button class="btn btn-sm ${allowance.is_archived ? "btn-success" : "btn-warning"}" 
                    onclick="window.allowancesPage.toggleArchive(${allowance.id}, ${allowance.is_archived})" 
                    title="${allowance.is_archived ? "Unarchive" : "Archive"}">
              ${allowance.is_archived ? "Unarchive" : "Archive"}
            </button>
            <button class="btn btn-sm btn-danger" onclick="window.allowancesPage.deleteAllowance(${allowance.id})" title="Delete">
              Delete
            </button>
          </div>
        </td>
  ...
  async showEditModal(id) {
    try {
      ...
      document.getElementById("allowance_code").value = this.currentAllowance.allowance_code
      document.getElementById("allowance_name").value = this.currentAllowance.allowance_name
      document.getElementById("is_percentage").value = this.currentAllowance.is_percentage
      document.getElementById("default_amount").value = this.currentAllowance.default_amount
      document.getElementById("description").value = this.currentAllowance.description || ""
  ...
  async saveAllowance() {
    ...
    try {
      ...
      if (this.currentAllowance) {
        response = await this.crudManager.update(this.currentAllowance.id, data)
      } else {
        response = await this.crudManager.create(data)
      }

      if (response.success) {
        this.crudManager.showMessage(response.message || "Allowance saved successfully!", "success")
        this.closeModal()
        await this.loadAllowances()
      } else {
        this.crudManager.showMessage(response.message || "Failed to save allowance", "error")
      }
    } catch (error) {
      console.error("[v0] Error saving allowance:", error)
      this.crudManager.showMessage("Failed to save allowance. Please try again.", "error")
    }
  }
  ...
  async toggleArchive(id, isArchived) {
    ...
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage(`Allowance ${action}d successfully!`, "success")
        await this.loadAllowances()
      } else {
        this.crudManager.showMessage(response.message || `Failed to ${action} allowance`, "error")
      }
    } catch (error) {
      console.error(`[v0] Error ${action}ing allowance:`, error)
      this.crudManager.showMessage(`Failed to ${action} allowance. Please try again.`, "error")
    }
  }

  async deleteAllowance(id) {
    if (!confirm("Are you sure you want to permanently delete this allowance? This action cannot be undone.")) {
      return
    }
    ...
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage("Allowance deleted successfully!", "success")
        await this.loadAllowances()
      } else {
        this.crudManager.showMessage(response.message || "Failed to delete allowance", "error")
      }
    } catch (error) {
      console.error("[v0] Error deleting allowance:", error)
      this.crudManager.showMessage("Failed to delete allowance. Please try again.", "error")
    }
  }
}

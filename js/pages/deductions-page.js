class DeductionsPage {
  constructor() {
    this.crudManager = new window.CRUDManager(window.API_ENDPOINTS.DEDUCTIONS, "deduction")
        ...
        <div class="card-header">
          ...
          <div style="display: flex; gap: 12px;">
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
              <input type="checkbox" id="showArchivedDeductions" ${this.showArchived ? "checked" : ""}>
              Show Archived
            </label>
  ...
  renderTable(deductions) {
    ...
    tbody.innerHTML = deductions
      ...
      <tr>
        ...
        <td><span class="badge badge-primary">${deduction.is_percentage ? "Percentage" : "Fixed"}</span></td>
        <td>${deduction.is_percentage ? deduction.default_amount + "%" : "$" + Number.parseFloat(deduction.default_amount).toFixed(2)}</td>
        <td>${deduction.description || "-"}</td>
        <td>
          <span class="badge ${deduction.is_archived ? "badge-danger" : "badge-success"}">
            ${deduction.is_archived ? "Archived" : "Active"}
          </span>
        </td>
        <td>
          <div class="action-buttons">
            <button class="btn btn-sm btn-primary" onclick="window.deductionsPage.showEditModal(${deduction.id})" title="Edit">
              Edit
            </button>
            <button class="btn btn-sm ${deduction.is_archived ? "btn-success" : "btn-warning"}" 
                    onclick="window.deductionsPage.toggleArchive(${deduction.id}, ${deduction.is_archived})" 
                    title="${deduction.is_archived ? "Unarchive" : "Archive"}">
              ${deduction.is_archived ? "Unarchive" : "Archive"}
            </button>
            <button class="btn btn-sm btn-danger" onclick="window.deductionsPage.deleteDeduction(${deduction.id})" title="Delete">
              Delete
            </button>
          </div>
        </td>
  ...
  async showEditModal(id) {
    try {
      ...
      document.getElementById("deduction_code").value = this.currentDeduction.deduction_code
      document.getElementById("deduction_name").value = this.currentDeduction.deduction_name
      document.getElementById("is_percentage").value = this.currentDeduction.is_percentage
      document.getElementById("default_amount").value = this.currentDeduction.default_amount
      document.getElementById("description").value = this.currentDeduction.description || ""
  ...
  async saveDeduction() {
    ...
    try {
      ...
      if (this.currentDeduction) {
        response = await this.crudManager.update(this.currentDeduction.id, data)
      } else {
        response = await this.crudManager.create(data)
      }

      if (response.success) {
        this.crudManager.showMessage(response.message || "Deduction saved successfully!", "success")
        this.closeModal()
        await this.loadDeductions()
      } else {
        this.crudManager.showMessage(response.message || "Failed to save deduction", "error")
      }
    } catch (error) {
      console.error("[v0] Error saving deduction:", error)
      this.crudManager.showMessage("Failed to save deduction. Please try again.", "error")
    }
  }
  ...
  async toggleArchive(id, isArchived) {
    ...
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage(`Deduction ${action}d successfully!`, "success")
        await this.loadDeductions()
      } else {
        this.crudManager.showMessage(response.message || `Failed to ${action} deduction`, "error")
      }
    } catch (error) {
      console.error(`[v0] Error ${action}ing deduction:`, error)
      this.crudManager.showMessage(`Failed to ${action} deduction. Please try again.`, "error")
    }
  }

  async deleteDeduction(id) {
    if (!confirm("Are you sure you want to permanently delete this deduction? This action cannot be undone.")) {
      return
    }
    ...
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage("Deduction deleted successfully!", "success")
        await this.loadDeductions()
      } else {
        this.crudManager.showMessage(response.message || "Failed to delete deduction", "error")
      }
    } catch (error) {
      console.error("[v0] Error deleting deduction:", error)
      this.crudManager.showMessage("Failed to delete deduction. Please try again.", "error")
    }
  }
}

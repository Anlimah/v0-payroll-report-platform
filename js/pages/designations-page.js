class DesignationsPage {
  constructor() {
    this.crudManager = new window.CRUDManager(window.API_ENDPOINTS.DESIGNATIONS, "designation")
        ...
        <div class="card-header">
          ...
          <div style="display: flex; gap: 12px;">
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px;">
              <input type="checkbox" id="showArchivedDesignations" ${this.showArchived ? "checked" : ""}>
              Show Archived
            </label>
  ...
  attachEventListeners() {
    document.getElementById("addDesignationBtn").addEventListener("click", () => this.openModal())
    document.getElementById("closeDesignationModal").addEventListener("click", () => this.closeModal())
    document.getElementById("cancelDesignationBtn").addEventListener("click", () => this.closeModal())
    document.getElementById("saveDesignationBtn").addEventListener("click", () => this.saveDesignation())
    document.getElementById("showArchivedDesignations").addEventListener("change", (e) => {
      ...
      if (designations.length > 0) {
        tbody.innerHTML = designations
          ...
          <tr>
            ...
            <td><span class="badge ${designation.is_archived ? "badge-danger" : "badge-success"}">${designation.is_archived ? "Archived" : "Active"}</span></td>
            <td>
              <div class="action-buttons">
                <button class="btn btn-sm btn-primary" onclick="window.designationsPage.editDesignation(${designation.id})">Edit</button>
                ${
                ...
                <button class="btn btn-sm btn-danger" onclick="window.designationsPage.deleteDesignation(${designation.id})">Delete</button>
              </div>
            </td>
          </tr>
        `,
  ...
  openModal(designation = null) {
    ...
    title.textContent = designation ? "Edit Designation" : "Add New Designation"
    form.reset()

    if (designation) {
      document.getElementById("designationCode").value = designation.designation_code
      document.getElementById("designationName").value = designation.designation_name
  ...
  async saveDesignation() {
    const designationCode = document.getElementById("designationCode").value.trim()
    ...
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage(response.message || "Designation saved successfully", "success")
        this.closeModal()
        await this.loadDesignations()
      } else {
        this.crudManager.showMessage(response.message || "Failed to save designation", "error")
      }
    } catch (error) {
      console.error("Error saving designation:", error)
      this.crudManager.showMessage("An error occurred while saving the designation", "error")
    }
  }
  ...
  async editDesignation(id) {
    try {
      ...
      this.crudManager.showMessage("Failed to load designation details", "error")
    }
  }
  ...
  async archiveDesignation(id) {
    this.crudManager.confirmArchive(async () => {
      try {
        ...
        if (response.success) {
          this.crudManager.showMessage("Designation archived successfully", "success")
          await this.loadDesignations()
        } else {
          this.crudManager.showMessage(response.message || "Failed to archive designation", "error")
        }
      } catch (error) {
        console.error("Error archiving designation:", error)
        this.crudManager.showMessage("An error occurred while archiving the designation", "error")
      }
    })
  }
  ...
  async unarchiveDesignation(id) {
    try {
      ...
      if (response.success) {
        this.crudManager.showMessage("Designation unarchived successfully", "success")
        await this.loadDesignations()
      } else {
        this.crudManager.showMessage(response.message || "Failed to unarchive designation", "error")
      }
    } catch (error) {
      console.error("Error unarchiving designation:", error)
      this.crudManager.showMessage("An error occurred while unarchiving the designation", "error")
    }
  }
  ...
  async deleteDesignation(id) {
    this.crudManager.confirmDelete(async () => {
      try {
        ...
        if (response.success) {
          this.crudManager.showMessage("Designation deleted successfully", "success")
          await this.loadDesignations()
        } else {
          this.crudManager.showMessage(response.message || "Failed to delete designation", "error")
        }
      } catch (error) {
        console.error("Error deleting designation:", error)
        this.crudManager.showMessage("An error occurred while deleting the designation", "error")
      }
    })
  }
}

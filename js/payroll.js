...
class PayrollManager {
  ...
  async loadCurrencyRate() {
    try {
      const response = await window.ApiService.get(API_ENDPOINTS.CURRENCY_RATES + "?current=true")
  ...
  async searchStaff(staffNumber) {
    try {
      const response = await window.ApiService.get(API_ENDPOINTS.STAFFS + `?staff_number=${staffNumber}`)
      if (response.success && response.data) {
  ...
  async savePayrollEntry(periodId, staffId, basicSalary, allowances, deductions) {
    const payrollData = {
    ...
    try {
      const response = await window.ApiService.post(API_ENDPOINTS.PAYROLL_ENTRIES, payrollData)
      return response
    } catch (error) {
  ...
  async updatePayrollEntry(entryId, basicSalary, allowances, deductions) {
    ...
    try {
      const response = await window.ApiService.put(API_ENDPOINTS.PAYROLL_ENTRIES, payrollData)
      return response
    } catch (error) {
